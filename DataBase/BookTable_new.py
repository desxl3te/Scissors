...
def test_book_table_success(db_connection):
    ...
    cursor.callproc('BookTable', [9001, 9001, 1, '2025-11-01 19:00:00', 3])
    db_connection.commit()

    cursor.execute("""
        SELECT id, table_id, user_id, event_id, guests_count, status 
        FROM reservations 
    ...
    assert booking[1] == 9001
    assert booking[2] == 9001
    assert booking[3] == 1
    assert booking[4] == 3
    assert booking[5] == 'confirmed'
...
def test_book_table_user_not_found(db_connection):
    ...
    try:
        cursor.callproc('BookTable', [9002, 99999, 1, '2025-11-01 19:00:00', 2])
        db_connection.commit()
        pytest.fail("Процедура не выбросила ошибку для несуществующего пользователя!")
    except pymysql.err.OperationalError as e:
        assert e.args[0] == 1644
        assert 'пользователь не найден' in str(e).lower()
...
def test_book_table_table_not_found(db_connection):
    ...
    try:
        cursor.callproc('BookTable', [99999, 9003, 1, '2025-11-01 19:00:00', 2])
        db_connection.commit()
        pytest.fail("Процедура не выбросила ошибку для несуществующего столика!")
    except pymysql.err.OperationalError as e:
        assert e.args[0] == 1644
        assert 'столик не найден' in str(e).lower()
        db_connection.rollback()
    finally:
...
def test_book_table_already_booked(db_connection):
    ...
    cursor.execute("""
        ...
        VALUES (9004, 9004, 1, '2025-11-01 19:00:00', 2, 'confirmed')
    """)
    db_connection.commit()

    try:
        cursor.callproc('BookTable', [9004, 9005, 1, '2025-11-01 19:00:00', 3])
        db_connection.commit()
        pytest.fail("Процедура не выбросила ошибку для занятого времени!")
    except pymysql.err.OperationalError as e:
        assert e.args[0] == 1644
        assert 'уже забронирован' in str(e).lower()
        db_connection.rollback()
    finally:
...
def test_book_table_invalid_guests_count(db_connection):
    """Проверка: отрицательное guests_count блокируется на уровне БД"""
    cursor = db_connection.cursor()
    ...
    try:
        cursor.callproc('BookTable', [9006, 9006, 1, '2025-11-01 19:00:00', -5])
        db_connection.commit()
        pytest.fail("CHECK constraint не сработал!")
